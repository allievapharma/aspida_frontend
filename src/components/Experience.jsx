import React from 'react';
import CountUp from 'react-countup';
import { useInView } from 'react-intersection-observer';

const Experience = () => {
  const { ref, inView } = useInView({
    triggerOnce: true, // Count only once
    threshold: 0.3,     // Trigger when 30% of section is visible
  });

  return (
    <section
      ref={ref}
      className="py-10 myFadeup bg-gray-100 sm:py-16 lg:py-24 relative overflow-hidden"
    >
      <div className="circle-team"></div>
      <div className="max-w-5xl px-4 mx-auto sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-3xl font-bold leading-tight text-black sm:text-4xl lg:text-5xl">
            Numbers tell our story
          </h2>
          <p className="mt-3 text-xl leading-relaxed text-gray-600 md:mt-8">
            Built on trust, quality, and consistent service, our journey is defined by measurable impact and long-term partnerships.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-8 mt-10 text-center lg:mt-24 sm:gap-x-8 md:grid-cols-3">
          {/* Years in business */}
          <div>
            <h3 className="font-bold text-7xl">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-blue-600">
                {inView && <CountUp end={6} duration={3} />}+
              </span>
            </h3>
            <p className="mt-4 text-xl font-medium text-gray-900">Years in business</p>
            <p className="text-base mt-0.5 text-gray-500">Creating the successful path</p>
          </div>

          {/* Projects delivered */}
          <div>
            <h3 className="font-bold text-7xl">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-blue-600">
                {inView && <CountUp end={4821} duration={3} />}+
              </span>
            </h3>
            <p className="mt-4 text-xl font-medium text-gray-900">Medicine delivered</p>
            <p className="text-base mt-0.5 text-gray-500">In last 6 years</p>
          </div>

          {/* Team members */}
          <div>
            <h3 className="font-bold text-7xl">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-blue-600">
                {inView && <CountUp end={37} duration={3} />}+
              </span>
            </h3>
            <p className="mt-4 text-xl font-medium text-gray-900">Team members</p>
            <p className="text-base mt-0.5 text-gray-500">Working for your success</p>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Experience;
